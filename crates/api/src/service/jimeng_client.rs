use crate::dto::jimeng_dto::{
    ...
    JimengSubmitRequest, JimengTaskHandle, JimengVideoMode, JimengVideoParams,
    VolcengineResponse, VALID_ASPECT_RATIOS, VALID_SECONDS,
};
...
const API_VERSION: &str = "2024-06-06";
...
impl Default for RetryConfig {
    fn default() -> Self {
        Self { max_retries: 5, initial_backoff: Duration::from_secs(2), max_backoff: Duration::from_secs(30), backoff_multiplier: 2.0 }
    }
}
...
impl JimengClient {
    pub fn new(access_key_id: String, secret_access_key: String, base_url: Option<String>) -> Self {
        let client = Client::builder().timeout(Duration::from_secs(120)).build().expect("HTTP client build");
        Self {
    ...
    pub fn with_retry_config(mut self, config: RetryConfig) -> Self { self.retry_config = config; self }

    // ========== Unified High-Level API ==========

    /// Create a video with unified parameters (resolution + mode auto-detected).
    pub async fn create_video(&self, params: JimengVideoParams) -> Result<JimengTaskHandle, ApiError> {
    ...
    pub async fn create_text_to_video(
        &self, prompt: String, aspect_ratio: String, seconds: i32, resolution: JimengResolution,
    ...
    pub async fn create_image_to_video(
        &self, prompt: String, image_base64: String, seconds: i32, resolution: JimengResolution,
    ) -> Result<JimengTaskHandle, ApiError> {
        self.create_video(JimengVideoParams {
            prompt, resolution, seconds,
            aspect_ratio: None,
    ...
    /// Poll task status
    pub async fn get_task_status(&self, handle: &JimengTaskHandle) -> Result<JimengResultData, ApiError> {
        self.poll_with_retry(&handle.req_key, &handle.task_id).await
    }

    // ========== Retry wrapper ==========

    async fn submit_with_retry(&self, request: JimengSubmitRequest) -> Result<JimengTaskHandle, ApiError> {
        let mut last_err = None;
        let mut backoff = self.retry_config.initial_backoff;
        for attempt in 0..=self.retry_config.max_retries {
            if attempt > 0 {
                tracing::warn!("Jimeng submit retry {}/{}, wait {:?}", attempt, self.retry_config.max_retries, backoff);
                tokio::time::sleep(backoff).await;
                backoff = Duration::from_secs_f64((backoff.as_secs_f64() * self.retry_config.backoff_multiplier).min(self.retry_config.max_backoff.as_secs_f64()));
            }
            match self.submit_once(&request).await {
                Ok(h) => return Ok(h),
                Err(e) if Self::is_retryable(&e) => { last_err = Some(e); continue; }
                Err(e) => return Err(e),
            }
        }
        Err(last_err.unwrap_or_else(|| ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed("Jimeng submit retries exhausted".into()))))
    }

    async fn poll_with_retry(&self, req_key: &str, task_id: &str) -> Result<JimengResultData, ApiError> {
        let mut last_err = None;
        let mut backoff = self.retry_config.initial_backoff;
        for attempt in 0..=self.retry_config.max_retries {
            if attempt > 0 {
        ...
        Err(last_err.unwrap_or_else(|| ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed("Jimeng poll retries exhausted".into()))))
    }
    ...
    async fn submit_once(&self, req: &JimengSubmitRequest) -> Result<JimengTaskHandle, ApiError> {
        ...
        let body = serde_json::to_string(req).map_err(|e| ApiError::InternalServerError(e.to_string()))?;
        tracing::info!("Jimeng submit: req_key={}, frames={}", req.req_key, req.frames);

        let text = self.signed_post("CVSync2AsyncSubmitTask", &body).await?;
        let resp = Self::parse_response::<JimengSubmitData>(&text)?;
        if resp.code != CODE_SUCCESS {
            return Err(ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed(
                format!("Jimeng API error: code={}, message={}", resp.code, resp.message))));
        }
        let data = resp.data.ok_or_else(|| ApiError::InfrastructureError(
            InfrastructureError::ExternalApiResponseParsingFailed("Missing data in submit response".into())))?;
        tracing::info!("Jimeng submitted: task_id={}", data.task_id);
        Ok(JimengTaskHandle { task_id: data.task_id, req_key })
    }
    ...
    async fn poll_once(&self, req_key: &str, task_id: &str) -> Result<JimengResultData, ApiError> {
        ...
        resp.data.ok_or_else(|| ApiError::InfrastructureError(
            InfrastructureError::ExternalApiResponseParsingFailed("Missing data in result response".into())))
    }

    // ========== Response parsing ==========

    fn parse_response<T: serde::de::DeserializeOwned>(text: &str) -> Result<JimengApiResponse<T>, ApiError> {
        if let Ok(w) = serde_json::from_str::<VolcengineResponse<T>>(text) {
            if let Some(result) = w.result { return Ok(result); }
            if let Some(meta) = &w.response_metadata {
                if let Some(err) = &meta.error {
                    ...
                    return Ok(JimengApiResponse { code, message: err.message.clone().unwrap_or_default(), data: None });
                }
            }
        }
        serde_json::from_str::<JimengApiResponse<T>>(text).map_err(|e| {
            ApiError::InfrastructureError(InfrastructureError::ExternalApiResponseParsingFailed(e.to_string()))
        })
    }
    ...
    async fn signed_post(&self, action: &str, body: &str) -> Result<String, ApiError> {
        ...
        let url = format!("{}/?Action={}&Version={}", self.base_url, action, API_VERSION);
        let qs = format!("Action={}&Version={}", action, API_VERSION);
        let ph = hex_sha256(body.as_bytes());
        let host = self.base_url.trim_start_matches("https://").trim_start_matches("http://").split('/').next().unwrap_or(VOLCENGINE_HOST);
        ...
        let auth = format!("HMAC-SHA256 Credential={}/{}, SignedHeaders={}, Signature={}", self.access_key_id, cs, sh, sig);

        let resp = self.client.post(&url)
            .header("Content-Type", "application/json").header("Host", host)
            .header("X-Date", &dt).header("Authorization", &auth)
            .body(body.to_string()).send().await
            .map_err(|e| ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed(e.to_string())))?;
        let status = resp.status();
        let text = resp.text().await.map_err(|e| ApiError::InfrastructureError(InfrastructureError::ExternalApiResponseParsingFailed(e.to_string())))?;
        if !status.is_success() { tracing::warn!("Jimeng HTTP {}: {}", status, &text[..text.len().min(300)]); }
        Ok(text)
    }
    ...
    fn validate_prompt(&self, p: &str) -> Result<(), ApiError> {
        if p.is_empty() { return Err(ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed("Prompt cannot be empty".into()))); }
        if p.len() > 800 { return Err(ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed(format!("Prompt too long: {} (max 800)", p.len())))); }
        Ok(())
    }
    fn validate_aspect_ratio(&self, r: &str) -> Result<(), ApiError> {
        if !VALID_ASPECT_RATIOS.contains(&r) { return Err(ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed(format!("Invalid aspect_ratio: {}", r)))); }
        Ok(())
    }
    fn validate_seconds(&self, s: i32) -> Result<(), ApiError> {
        if !VALID_SECONDS.contains(&s) { return Err(ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed(format!("Invalid seconds: {} (must be 5 or 10)", s)))); }
        Ok(())
    }

    pub fn encode_image(data: &[u8]) -> String { BASE64_STANDARD.encode(data) }
}
...
fn hex_hmac_sha256(key: &[u8], data: &[u8]) -> String { hex_encode(&hmac_sha256(key, data)) }
fn hex_sha256(data: &[u8]) -> String { use sha2::Digest; let mut h = Sha256::new(); h.update(data); hex_encode(&h.finalize()) }
fn hex_encode(b: &[u8]) -> String { b.iter().map(|x| format!("{:02x}", x)).collect() }

pub fn is_jimeng_model(model_key: &str) -> bool { model_key.starts_with("jimeng-") }
...
mod tests {
    ...
    #[test] fn test_client_new() { let c = JimengClient::new("k".into(),"s".into(),None); assert!(c.base_url.contains(VOLCENGINE_HOST)); }
    #[test] fn test_is_jimeng()  { assert!(is_jimeng_model("jimeng-video-3.0-720p")); assert!(!is_jimeng_model("sora-2")); }
    #[test] fn test_sha256()     { assert_eq!(hex_sha256(b""), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"); }
    #[test] fn test_encode_img() { assert_eq!(JimengClient::encode_image(&[0x89,0x50]), "iVA="); }
    ...
    #[test] fn test_retryable() {
        let yes = ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed("code=50430".into()));
        assert!(JimengClient::is_retryable(&yes));
        let no = ApiError::InfrastructureError(InfrastructureError::ExternalApiRequestFailed("code=50400".into()));
    ...
    fn test_bmp_256() -> Vec<u8> {
        ...
        let rs = ((w * 3 + 3) / 4) * 4; let ps = rs * h; let fs = 54 + ps;
        let mut b = Vec::with_capacity(fs as usize);
        b.extend_from_slice(b"BM"); b.extend(&(fs as u32).to_le_bytes()); b.extend(&[0u8;4]); b.extend(&54u32.to_le_bytes());
        b.extend(&40u32.to_le_bytes()); b.extend(&(w as i32).to_le_bytes()); b.extend(&(h as i32).to_le_bytes());
        b.extend(&1u16.to_le_bytes()); b.extend(&24u16.to_le_bytes()); b.extend(&0u32.to_le_bytes());
        b.extend(&(ps as u32).to_le_bytes()); b.extend(&2835u32.to_le_bytes()); b.extend(&2835u32.to_le_bytes());
        b.extend(&0u32.to_le_bytes()); b.extend(&0u32.to_le_bytes());
        let pad = (rs - w * 3) as usize;
        for y in (0..h).rev() { for x in 0..w { let i=((y*w+x)*3) as usize; b.push(rgb[i+2]); b.push(rgb[i+1]); b.push(rgb[i]); } b.extend(std::iter::repeat(0u8).take(pad)); }
        b
    }

    async fn poll_done(c: &JimengClient, h: &JimengTaskHandle, max: usize, interval: u64) -> Result<JimengResultData, String> {
        for i in 1..=max {
            tokio::time::sleep(Duration::from_secs(interval)).await;
            match c.get_task_status(h).await {
                Ok(r) => { println!("    [poll {}/{}] status={} video={}", i, max, r.status, r.get_video_url().is_some()); if r.is_done() || r.is_failed() { return Ok(r); } }
                Err(e) => { println!("    [poll {}/{}] err: {:?}", i, max, e); }
            }
        }
    ...
    async fn test_real_jimeng_all_products() {
        ...
        for res in ALL_RESOLUTIONS {
            for (mode, prompt, img, end_img) in modes {
                ...
                match handle_result {
                    Ok(handle) => {
                        ...
                        match poll_done(&client, &handle, poll_max, 5).await {
                            Ok(r) if r.is_done() => {
                                ...
                                results.push(R { label, req_key, task_id: handle.task_id, ok: true, url, err: String::new() });
                            }
                            Ok(r) => {
                    ...
                    Err(e) => {
                        let msg = format!("{:?}", e);
                        let short = if let Some(i) = msg.find("code=") { msg[i..msg.len().min(i+80)].to_string() } else { msg.chars().take(80).collect() };
                        println!("  ✗ submit: {}", short);
                        results.push(R { label, req_key, task_id: String::new(), ok: false, url: String::new(), err: short });
                    }
                }
        ...
        if perm_denied > 0 {
            println!("\n⚠ {} combination(s) returned 50400 Access Denied.", perm_denied);
        ...
        assert_eq!(real_fail, 0,
            "SDK failures (non-permission): {}. Permission denied: {}. Passed: {}.",
            real_fail, perm_denied, pass);
        assert!(pass >= 3,
            "At least 720P T2V/I2V + 1080P T2V should pass. Got {} passes.", pass);
    }
}
