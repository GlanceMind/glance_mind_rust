...
pub fn routes() -> Router<UserState> {
    Router::new()
        ...
        .route("/conversations/:id", patch(ai_chat_handler::update_conversation))
        .route("/conversations/:id", delete(ai_chat_handler::delete_conversation))
        .route("/conversations/:id/messages", get(ai_chat_handler::get_messages))
        .route("/conversations/:id/messages", post(ai_chat_handler::send_message))
        .route("/plans/:id/confirm", post(ai_chat_handler::confirm_plan))
        .route("/plans/:id/cancel", post(ai_chat_handler::cancel_plan))
        .route("/plans/:plan_id/steps/:step_id", patch(ai_chat_handler::update_plan_step))
}
