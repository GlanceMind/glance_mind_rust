...
impl OpenMontageJobStore for PgJobStore {
    ...
    fn append_event(&self, event: NewJobEvent) -> Result<AppendResult, String> {
        ...
        // Wrap event insert + job update in a transaction for atomicity, so a
        // partial failure can never leave an event row without its corresponding
        // job-row bump (or vice versa).
